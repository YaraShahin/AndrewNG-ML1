#multiple logistic regression non-normalized regularized using gradient descent
import numpy as np
...
def compute_cost(X, y, w, b, lamda = 1):
    ...
    for i in range(m):
        ...
        term1+= -y[i]*np.log(f_wb) - (1-y[i])*np.log(1-f_wb)
    term1/=m
...
def compute_gradient(X, y, w, b, lamda):
    ...
    dj_db = 0
    for i in range (m):
        diff_i = sigmoid(np.dot(X[i], w)+b)-y[i]
        dj_db += diff_i
        for j in range(n):
